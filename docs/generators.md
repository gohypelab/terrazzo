...
When `--bundler=vite`, the installer requires `vite_rails` because the generated admin layout uses Vite Rails asset helpers. Add `gem "vite_rails"` and run `bundle exec vite install` first so `config/vite.json` exists, or pass `--bundler=esbuild`.
