...
RSpec.describe Terrazzo::HasManyPagination do
  describe ".extract" do
    it "returns an empty hash when no hm_<attr>_page params are present" do
    ...
    it "extracts a single hm_<attr>_page param into a nested options hash" do
      result = described_class.extract({ "hm_orders_page" => "3" }, [:orders])
      expect(result).to eq({ orders: { _page: 3 } })
    end

    it "extracts multiple hm_<attr>_page params independently" do
    ...
    it "coerces the page value to an integer" do
      result = described_class.extract({ "hm_orders_page" => "7" }, [:orders])
      expect(result[:orders][:_page]).to eq(7)
    end

    it "ignores hm_<attr>_page params for attributes not in the allowed list" do
      result = described_class.extract({ "hm_orders_page" => "2", "hm_other_page" => "3" }, [:orders])
      expect(result).to eq({ orders: { _page: 2 } })
    end

    it "accepts symbol keys" do
      result = described_class.extract({ hm_orders_page: "4" }, [:orders])
